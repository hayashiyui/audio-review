...
- 対訳はサブディレクトリ `en/` に配置し、ファイル名は元記事と同一（例: `src/content/reviews/en/foo.mdx`、`src/content/columns/en/bar.mdx`）。
 - 画像参照は Vite alias を採用（`@assets/images/...`）。相対パスの `../../` 差は不要。
...
- 追加: `vite.resolve.alias` に `@` / `@assets` を定義
- 更新済 `src/content.config.ts`（`locale`/`translationKey` を `reviews`/`columns` に追加）
- 更新済 `tsconfig.json` の `paths`（`@assets/*` を追加）
...
**🎉 実装完了（2025-09-16更新）**: 全フェーズが完了！多言語化実装が成功し、日本語・英語両方でのサイト運用が可能。英語コンテンツ（reviews: 43記事、columns: 1記事）、hreflang、言語切り替えUI、RSS多言語対応、Pagefind言語別インデックス全て実装完了。ヘッダー言語切り替え、View Transitions エラー修正も完了。プレビュー確認済み。

Phase 1：設定とスキーマ
- [x] `astro.config.ts` に i18n と sitemap の i18n 設定を追加
- [x] `src/content.config.ts` に `locale`/`translationKey` を追加
- [x] `src/lib/i18n.ts` を追加
- [x] `package.json` の `postbuild` から `--force-language ja` を削除

Phase 2：ページ実装
- [x] `src/pages/en/{reviews,columns,tags,authors,blog}` を追加（ラッパーで `locale==='en'` を適用）
- [x] 詳細ページに対訳リンクと `hreflang` を出力
- [x] ヘッダーに言語トグル（対訳があれば直リンク、なければトップ）

Phase 3：コンテンツ整備
- [x] 代表記事を `en/` サブディレクトリへ複製・翻訳し `locale: en` と `translationKey` を付与 *(reviews: 43記事、columns: 1記事)*

Phase 4：検索（Pagefind）
- [x] `<html lang>` を各ページで正確に設定 *(英語: `lang="en"` 完了、日本語: SITE.localeフォールバックのため明示的設定が必要)*
- [x] `pnpm build` で言語別インデックス生成を確認
- [x] `SearchModal.astro` の UI 文言が lang に追従することを確認
- [ ] （任意）言語横断検索トグルを検証

Phase 5：SEO/RSS/検証
- [x] サイトマップの多言語 `<xhtml:link>` を確認
- [x] `/rss.xml` と `/en/rss.xml` の配信と内容（言語フィルタ）を確認
- [x] 主要ページで `hreflang` と canonical を手検証
